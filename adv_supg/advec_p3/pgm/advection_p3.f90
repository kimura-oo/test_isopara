...
! --- velocity, phi
      double precision, allocatable :: us(:), vs(:), ff(:), ffend(:)
! --- For Matrix
...
!
      ...
      allocate(ad(node), ea(10,10,nelm), buvp(node))
...
subroutine datain
      ...
      implicit none
      integer :: i, j, k, n, m
      character(80) :: inpfile, mesfile, uvsfile, bdcfile, finfile, resfile, outfile100, mass_conservation
...
!
      ...
      allocate( shp(10,mgp,nelm), shpx(10,mgp,nelm), shpy(10,mgp,nelm) ) !set allocation in end as possible as u can
!
...
subroutine makeSSF
	...
	double precision, allocatable :: dxdxi(:,:), dxidx(:,:)
      ...
      allocate(dxdxi(2,2), dxidx(2,2), sqd(2,10,mgp), L(3), N_vals(10), dNdL(10,3), dLdxi(3,2), dNdxi(10,2), n_local(10))
      ! allocate( shp(10,mgp,nelm), shpx(10,mgp,nelm), shpy(10,mgp,nelm) ) 
      ! allocate( jac(mgp,nelm) )
...
subroutine makmat
      ...
      allocate(Me(10,10), Ke(10,10), Ke_supg(10,10), Me_supg(10,10), E_elem(10,10))
      ...
      do m = 1, nelm
            ...
            do j = 1, 10
                  ...
                  ad(n_local(j)) = ad(n_local(j)) + E_elem(j,j) ! Assembling diagonal for scaling (subroutine dscale)
            end do
      end do
...
subroutine mkbuvp ! Right-hand side vector for Crank-Nicolson
      ...
      allocate(Me(10,10), Ke(10,10), Me_supg(10,10), Ke_supg(10,10))
      allocate(phi_e_n(10), be_M(10), be_K(10), be(10))
      allocate(n_local(10))
...
subroutine bicgst(eps, kx, nx, ad, k, xx, bb, mx, nc, ea, ifbc, nfbc)
      ...
      integer, intent(in) :: nfbc(ifbc), nc(10,mx)
      double precision, intent(inout) :: ad(nx), ea(10,10,mx)
...
!
      ...
      allocate( rr(nx), pp(nx), tt(nx), rs(nx), Ap(nx), At(nx), p(10) )
!
      do n = 1, nx
            bb(n) = bb(n) * ad(n)
            xx(n) = xx(n) / ad(n)
...
subroutine write_vtk(filename, xx, ff, nc, node, nelm, us, vs)
      ...
      !
      write(vtk_unit,'(A, 1X, I0, 1X, I0)') 'CELLS', nelm, nelm*11
      do m = 1, nelm
            write(vtk_unit,'(11I8)') 10, (nc(i,m)-1, i=1,10)      
      end do

      !
      write(vtk_unit,'(A, 1X, I0)') 'CELL_TYPES', nelm
      do m = 1, nelm
            write(vtk_unit,'(I8)') 69
      end do
