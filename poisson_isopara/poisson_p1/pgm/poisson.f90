...
program main
	...
	write(*,*) 'main program start! (Linear Elements)'
	call CPU_TIME(begin_time)
      
      call datain
      call set_integration_points
	call makeSSF 
      write(6,*)'Finish subroutine makeSSF'
	
      call makeLHS
      write(6,*)'Finish subroutine makeLHS'
	
      call set_integration_points
      write(6,*)'Finish subroutine set_integration_points'
      
      call set_line_integration_points
	call makeRHS
      write(6,*)'Finish subroutine makeRHS'
      
      call add_source_term
      write(6,*)'Finish subroutine add_source_term' 
	
      call boundc
      write(6,*)'Finish subroutine boundc'
	
      call makeLHS
      write(6,*)'Finish subroutine makeLHS'
	
      call sweep
      write(6,*)'Finish subroutine sweep'
...
!  
      ...
      ! double precision :: N_vals(3)    ! Shape function values N(L1,L2,L3)
      ! double precision :: dNdL(3,3)    ! dN/dL: Derivatives of N w.r.t. Area Coords
      ! double precision :: dLdxi(3,2)
      ! double precision :: dNdxi(3,2)   ! dN/d_xi, dN/d_eta: Final derivatives in reference coords
	
      dLdxi(1,1) = -1.0d0; dLdxi(1,2) = -1.0d0
      dLdxi(2,1) =  1.0d0; dLdxi(2,2) =  0.0d0
      dLdxi(3,1) =  0.0d0; dLdxi(3,2) =  1.0d0
      
      !allocate( dxdxi(2,2), dxidx(2,2), sqd(2,3,mgp) )
...
!   PURPOSE: Write results to a .vtk file for visualization in ParaView.
! ===================================================================
subroutine output_vtk
      use indata
      implicit none
    ...
    ! --- VTK File Header ---
      ...
      write(70,'(a)') '2D FEM Analysis Result for Poisson Equation'
      write(70,'(a)') 'ASCII'
      write(70,'(a)') 'DATASET UNSTRUCTURED_GRID'
      write(70,*)

    ! --- 1. Point (Node) Coordinates ---
    ! All node coordinates are written here. Z-coordinate is set to 0.0 for 2D.
      write(70,'(a,i0,a)') 'POINTS ', node, ' double'
      do m = 1, node
            write(70,'(3f18.9)') xx(1, m), xx(2, m), 0.0d0
      end do
      write(70,*)

    ! --- 2. Cell (Element) Connectivity ---
    ! Defines each element by listing its node indices.
    ! Each line: [num_nodes] [node1_idx] [node2_idx] [node3_idx]
    ! VTK uses 0-based indexing, so we subtract 1 from Fortran's 1-based indices.
      write(70,'(a,2i8)') 'CELLS ', nelm, nelm * 4
      do m = 1, nelm
            write(70,'(4i8)') 3, nc(1,m)-1, nc(2,m)-1, nc(3,m)-1
      end do
      write(70,*)

    ! --- 3. Cell Types ---
    ! Specifies the geometric type of each cell.
    ! VTK_TRIANGLE has a type code of 5.
    ...
    ! --- 4. Point Data (Nodal Results) ---
    ! This section contains scalar/vector data associated with points.
    ...
    ! Output scalar potential (phi)
      ...
      do m = 1, node
            write(70,'(f18.9)') phi(m)
      end do
      write(70,*)

    ! --- 5. Cell Data (Elemental Results) ---
    ! This section contains scalar/vector data associated with cells.
      write(70,'(a,i8)') 'CELL_DATA ', nelm
    ! Output vector velocity (ue, ve)
      write(70,'(a)') 'VECTORS Velocity double'
      do m = 1, nelm
            write(70,'(3f18.9)') ue(m), ve(m), 0.0d0
      end do
      write(70,*)
